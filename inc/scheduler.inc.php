...
// Check if the scheduler is turned on
$scheduler_is_enabled = dev_scheduler_status();

// Do not run the scheduler if it is disabled
if($scheduler_is_enabled === true)
{
...
$timestamp        = sanitize(time(), 'int', 0);
$maintenance_log  = ($scheduler_is_enabled === true) ? 'maintenance' : 'maintenance_off';
query(" INSERT INTO logs_scheduler
        SET         logs_scheduler.happened_at      = '$timestamp'        ,
                    logs_scheduler.task_type        = '$maintenance_log'  ");
