...
NoBleme is a fully handcrafted website, developed entirely from scratch by {{link|todo_link|Éric Bisceglia}} (known online as {{link|pages/users/1|Bad}}). Ever since its launch in 2005, NoBleme's policy on development has been to be as public as possible: its {{external|https://github.com/EricBisceglia/NoBleme.com|source code}} is publicly available, its {{link|pages/tasks/roadmap|roadmap}} allows you to follow the development progress, {{link|pages/dev/blog_list|devblogs}} keep you updated on what is going on, and this very page does its best to explain which philosophies and technologies are used in NoBleme's development.
EOT
);
