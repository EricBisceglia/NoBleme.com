...
NoBleme is a fully handcrafted website, developed entirely from scratch by {{link|cv|Éric Bisceglia}} (known online as {{link|pages/users/1|Bad}}). Ever since its launch in 2005, NoBleme's policy on development has been to be as public as possible: its {{external|https://github.com/EricBisceglia/NoBleme.com|source code}} is publicly available, its {{link|pages/tasks/roadmap|roadmap}} allows you to follow the development progress, {{link|pages/dev/blog_list|devblogs}} keep you updated on what is going on, and this very page does its best to explain which philosophies and technologies are used in NoBleme's development.
EOT
);
___('doc_bts_intro',    'FR', <<<EOT
NoBleme est un site fabriqué artisanalement à partir de rien, développé intégralement par {{link|cv|Éric Bisceglia}} (utilisant sur Internet le pseudonyme {{link|pages/users/1|Bad}}). Depuis sa création en 2005, le développement de NoBleme a toujours été délibérément aussi public que possible : le {{external|https://github.com/EricBisceglia/NoBleme.com|code source}} est disponible publiquement, le {{link|pages/tasks/roadmap|plan de route}} vous permet de suivre le progrès, les {{link|pages/dev/blog_list|devblogs}} vous tiennent à jour, et cette page vous explique les philosophies et technologies utilisées dans le développement de NoBleme.
EOT
);
